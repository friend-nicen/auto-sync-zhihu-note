 ...
 * 加载主题设置页面
 * */
function nicen_sync_setting_load( $options ) {
                ...
                >
                    ...
                    <div class="card-container">
                        <a-tabs type="card" v-model="activeKey" @change="change">
							<?php
							//输出输入域
							nicen_sync_do_settings_sections_user( $plugin_page );
							?>
                        </a-tabs>
                    </div>
                </a-form>
 ...
 * */
function nicen_sync_do_settings_fields_user( $page, $section, $callback = false ) {
 ...
 * 主题设置片段页面输出
 * */
function nicen_sync_do_settings_sections_user( $page ) {
	...
	foreach ( (array) $wp_settings_sections[ $page ] as $key => $section ) {
		...
		nicen_sync_do_settings_fields_user( $page, $section['id'], $section['callback'] ?? false );
 ...
 * 数字输入框
 * */
function nicen_sync_form_number( $args ) {
	?>
    <a-input-number
 ...
 * 基础输入框
 * */
function nicen_sync_form_input( $args ) {
	?>
    <a-input
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            placeholder="请输入<?php echo esc_html( $args['title'] ); ?>"
 ...
 * 基础密码输入框
 * */
function nicen_sync_form_password( $args ) {
	?>
    <a-input-password
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            placeholder="请输入<?php echo esc_html( $args['title'] ); ?>"
 ...
 * 基础开关
 * */
function nicen_sync_form_switch( $args ) {
	?>
 ...
 * 基础开关
 * */
function nicen_sync_form_textarea( $args ) {
	?>
    <a-textarea
            name="<?php echo esc_html( $args['label_for'] ); ?>"
            v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"
 ...
 * 基础开关
 * */
function nicen_sync_form_color( $args ) {
	?>
 ...
 * 文字说明
 * */
function nicen_sync_form_text( $args ) {
	?>
 ...
 * 单选
 * */
function nicen_sync_form_select( $args ) {
	?>
    <input name="<?php echo esc_html( $args['label_for'] ); ?>"
           v-model="data.<?php echo esc_html( $args['label_for'] ); ?>"
           hidden/>
 ...
 * 单选
 * */
function nicen_sync_form_multi( $args ) {
 ...
 * 媒体选择器
 * */
function nicen_sync_form_media( $args ) {
	?>
